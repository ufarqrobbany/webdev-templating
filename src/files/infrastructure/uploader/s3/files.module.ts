    ...
    MulterModule.registerAsync({
      ...
      useFactory: async (configService: ConfigService<AllConfigType>) => {
        ...
        try {
          ...
          const bucketName = configService.getOrThrow(
          ...
          if (data.Name) {
            console.log(
              `✅ S3/R2 CONNECTION SUCCESSFUL. Key has read access to bucket: ${data.Name}.`,
            );
          } else {
          ...
          console.error(
            '❌ S3/R2 CONNECTION FAILED!!! (Check Key, Secret, and BUCKET PERMISSIONS)',
          );
          console.error('Error Details:', error.name, error.message);

          throw new Error(
            'R2/S3 Connectivity Test Failed. See console for details.',
          );
        }
